...
namespace UnrealUtilities
{
    ...
    {
        /// <summary>Locates the named plugin's Marketplace installation beneath an explicit engine root.</summary>
        public static string GetMarketplacePluginPath(Engine engine, string pluginName)
        {
            return Path.Combine(engine.TargetPath, @"Engine\Plugins\Marketplace", pluginName);
        }
