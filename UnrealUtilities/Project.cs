...
namespace UnrealUtilities
{
    ...
    public class Project : IEngineInstanceProvider
    {
        ...
        /// <summary>Resolves the required descriptor association against installed engines, failing if unavailable.</summary>
        public Engine EngineInstance
        {
            get
            {
                // Empty metadata cannot select an arbitrary installation for an engine-required operation.
                string association = RequireText(ProjectDescriptor.EngineAssociation, nameof(ProjectDescriptor.EngineAssociation), "Project engine association is required.");
                return EngineFinder.GetEngineInstall(association);
            }
        }

        /// <summary>Gets the authored engine identity without resolving machine installation state.</summary>
        public string EngineInstanceName => ProjectDescriptor.EngineFriendlyName;
        ...
        /// <summary>Deletes recursive Unreal intermediate directories while leaving authored project data intact.</summary>
        public void CleanIntermediateDirectories()
        {
            foreach (string path in Directory.GetDirectories(ProjectPath, "Intermediate", SearchOption.AllDirectories))
            {
        ...
        /// <summary>Removes matching project-contained plugin directories.</summary>
        ...
        /// <summary>Removes source files and persisted Unreal module declarations; reload explicitly to refresh the descriptor snapshot.</summary>
        public void ConvertToBlueprintOnly()
        {
            ...
            JObject descriptor = JObject.Parse(File.ReadAllText(UProjectPath));
            descriptor.Remove("Modules");
            File.WriteAllText(UProjectPath, descriptor.ToString());
        }
