...
namespace UnrealUtilities
{
    ...
    public class TestReport
    {
        ...
        /// <summary>Uses Unreal's report failure count as the authoritative run outcome.</summary>
        [JsonIgnore]
        public bool HasFailures => Failed > 0;

        /// <summary>Loads the required report for a supported engine; unsupported engines return no report.</summary>
        public static TestReport? ReadRequiredReport(string filePath, Engine engine)
        {
            if (!engine.SupportsTestReports)
            {
                return null;
            }

            return Load(filePath) ?? throw new FileNotFoundException("Expected automation test report.", filePath);
        }
